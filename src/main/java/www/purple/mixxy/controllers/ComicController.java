...
public class ComicController {
   ...
   *
   * Pass in the id of the parent comic to remix on along with the current user,
   * context and data model information.
   *
   * @param id
   ...
   *          controller method fields), bean violations (on an injected beans
   *          field) or general violations (deprecated)
   *
  ...
  /**
   * Returns the {@link Result} containing the actual image data for a
   * {@link Comic}. May be accessed through the {@code /api/} prefix for
   * consistency, but the behavior for this case is identical, unlike that of
   * the other routes. An image can be accessed with a plain old HTTP request,
   * and will be returned to the client as its constituent bytes (as opposed to
   * some encoding like base64).
   *
   * Whether this image comes from DeviantArt or from Mixxy should be irrelevant
   * to the user of this route.
   *
   * @return A {@link Result} containing the relevant {@link Comic}'s image
   *         data.
   */
