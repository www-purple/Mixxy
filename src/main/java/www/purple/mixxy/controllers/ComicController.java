...
public class ComicController {
  ...
  public Result newRemix(@PathParam("user") String user, @PathParam("work") String slug,
      Context context,
   ...
   */
  @FilterWith(JsonEndpoint.class)
  public Result newWork(@LoggedInUser String username, Context context,
      @JSR303Validation ComicDto comicDto,
   ...
   */
  @FilterWith(JsonEndpoint.class)
  public Result postWork(@LoggedInUser String username, Context context,
      @Param("muroimage") String muroimage,
  ...
  @FilterWith(JsonEndpoint.class)
  public Result delete(@PathParam("user") String username, @PathParam("work") String slug,
      Context context) {
  ...
  @FilterWith(JsonEndpoint.class)
  public Result update(@PathParam("user") String username, @PathParam("work") String slug,
      Context context) {
  ...
  /**
   * Returns the {@link Result} containing the actual image data for a {@link Comic}. May be
   * accessed through the {@code /api/} prefix for consistency, but the behavior for this case is
   * identical, unlike that of the other routes. An image can be accessed with a plain old HTTP
   * request, and will be returned to the client as its constituent bytes (as opposed to some
  ...
  public Result series(@PathParam("user") String user, @PathParam("series") String series) {
    ...
    return Results.ok().render("comics", comics).render("user", user).html().render("series",
        series);
  }
