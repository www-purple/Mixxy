...
public class ComicTest extends NinjaAppengineBackendTest {
  ...
  public void testCreateComic() {
    ...
    // Create a new comic
    Comic comic = new Comic(null, yetAnotherBob, "cool title", "interesting description",
        "who cares", null);
    ...
    User user = ofy.load().type(User.class).filter("username", yetAnotherBob.username).first()
        .now();
    Comic recievedComic = ofy.load().type(Comic.class).filter("authorId", user.id)
        .filter("sluggedTitle", comic.sluggedTitle).first().now();
